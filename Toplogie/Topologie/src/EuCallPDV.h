...
class EuCallPDV: public vecSpace {
public:
	EuCallPDV();
	EuCallPDV(realSpace T,realSpace X, double sigmaL, double sigmaH, double r, double K);
	virtual ~EuCallPDV();
	...
	if(low){
		return alpha1(t,x,low) - sigmaL*sqrt(final(0)-t) ;}
	else{
		return alpha1(t,x,low) - sigmaH*sqrt(final(0)-t);
		}
	}
//	PDV delta : new price in the short memory path, computes heuristic and chooses between high or low sigma
	double Delta(const double& t,const double& x,arma::vec& longPath,arma::vec& shortPath, arma::vec& vol, int i);
